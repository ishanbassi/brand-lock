...
@Component({
  selector: 'app-trademark-hearing',
  imports: [CommonModule, ReactiveFormsModule, RouterModule, PhoneInputComponent],
  templateUrl: './trademark-hearing.component.html',
  styleUrl: './trademark-hearing.component.scss'
})
export class TrademarkHearingComponent implements OnInit {
  ...
  submit() {
    ...
    const lead: NewLead = {
      ...
      city: fv.city!,
      comments: `TRADEMARK HEARING | TM No: ${fv.trademarkNumber}`
    };
    ...
    this.leadService.create(lead)
      ...
      .subscribe({
        next: (response) => {
          ...
          //   queryParams: { service: 'TRADEMARK_HEARING', lead_id: response.body?.id, tm_number: fv.trademarkNumber }
          // });
        },
