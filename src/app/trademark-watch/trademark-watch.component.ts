...
@Component({
  selector: 'app-trademark-watch',
  imports: [CommonModule, ReactiveFormsModule, RouterModule, PhoneInputComponent],
  templateUrl: './trademark-watch.component.html',
  styleUrl: './trademark-watch.component.scss'
})
export class TrademarkWatchComponent implements OnInit {
  ...
  submit() {
    ...
    const lead: NewLead = {
      ...
      city: fv.city!,
      comments: `TRADEMARK WATCH SERVICE | Brand: ${fv.brandName}`
    };
      ...
      .subscribe({
        next: (response) => {
          ...
          this.router.navigate(['/service-checkout'], {
            queryParams: { service: 'TRADEMARK_WATCH', lead_id: response.body?.id }
          });
        },
