...
@Component({
  ...
  styleUrl: './trademark-objection-reply.component.scss'
})
export class TrademarkObjectionReplyComponent implements OnInit {
  ...
  submit() {
    ...
    const lead: NewLead = {
      ...
      city: fv.city!,
      comments: `TRADEMARK OBJECTION REPLY | TM No: ${fv.trademarkNumber}`
    };
      ...
      .subscribe({
        next: (response) => {
          ...
          this.router.navigate(['/service-checkout'], {
            queryParams: { service: 'TRADEMARK_OBJECTION', lead_id: response.body?.id, tm_number: fv.trademarkNumber }
          });
        },
