...
export interface IFilterOptions {
  ...
  removeCompleteFilter(name: string): boolean;
}
...
export class FilterOptions implements IFilterOptions {
  ...
  removeCompleteFilter(name: string): boolean {
    if (this.removeFilterOptionByName(name)) {
      this.changed();
      return true;
    }
    return false;
  ...
  protected removeFilterOptionByName(name: string): boolean {
    const indexOf = this._filterOptions.findIndex(thisOption => thisOption.name === name);
    if (indexOf === -1) {
      return false;
    }

    this._filterOptions.splice(indexOf, 1);
    return true;
  }
}
