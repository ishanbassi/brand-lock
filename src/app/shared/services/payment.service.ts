...
import { IPayment, NewPayment } from '../../../models/payment.model';
...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'createdDate' | 'modifiedDate'> & {
...
export class PaymentService {
 ...
 createPaymentFromTrademark(tmId: number): Observable<EntityResponseType> {
  ...
  addPaymentToCollectionIfMissing<Type extends Pick<IPayment, 'id'>>(
    ...
    const payments: Type[] = paymentsToCheck.filter(x => x != null);
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...
      modifiedDate: payment.modifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...
      modifiedDate: restPayment.modifiedDate ? dayjs(restPayment.modifiedDate) : undefined,
    };
  }
